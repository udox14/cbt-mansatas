...
export interface MapelResultsData {
  exam_id?: string;
  exam_title: string;
  subject_name?: string;
  room_name?: string;
...
export async function generateExamResultsDocx(options: ExamResultsDocxOptions): Promise<Blob> {
  ...
  const sections = options.mapels.map((mapel) => {
    const examTitle = mapel.subject_name || mapel.exam_title || 'HASIL UJIAN';
    const roomName = mapel.room_name || 'Semua Ruangan / Kelas';

    // Sort results for this mapel
    const sortedResults = [...(mapel.results || [])].sort((a, b) => {
      if (sortBy === 'score') {
      ...
      return new TableRow({
        ...
        children: [
          ...
          new TableCell({
            ...
            verticalAlign: VerticalAlignTable.CENTER,
            shading: { fill: bgFill },
            children: [new Paragraph({ spacing: { before: 10, after: 10 }, children: [new TextRun({ text: item.full_name, size: 18, font: 'Arial' })] })],
          }),
          new TableCell({
            width: { size: COLUMN_WIDTHS[3], type: WidthType.DXA },
            verticalAlign: VerticalAlignTable.CENTER,
