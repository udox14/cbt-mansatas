...
function AdminContent() {
  ...
  const menu: { key: Page; label: string; icon: string }[] = [
    ...
    { key: 'peserta', label: 'Peserta Tes', icon: '👥' },
    { key: 'rooms', label: 'Ruangan & Proktor', icon: '🏫' },
    { key: 'pelaksana', label: 'Pelaksana Tes', icon: '🛡️' },
  ];
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-60 bg-white border-r border-surface-100
        ...
        <div className="p-4 border-b border-surface-100">
          <div className="flex items-center gap-2">
            ...
            <div><p className="text-sm font-bold text-surface-900">CBT Admin</p><p className="text-[10px] text-surface-400">{user.full_name}</p></div>
          </div>
        </div>
...
// ══════════════════════════════════════════════════════════════
// PESERTA TES — gabungan pendaftar PMB + peserta non-PMB
// ══════════════════════════════════════════════════════════════
function PesertaPage() {
  ...
  useEffect(() => {
    GET<Pendaftar[]>('/api/admin/pendaftar').then(r => { if (r.success) setPendaftar(r.data || []); setLoading(false); });
  }, []);

  const filtered = filter ? pendaftar.filter(p => p.ruang_tes === filter) : pendaftar;
  const rooms = Array.from(new Set(pendaftar.map(p => p.ruang_tes).filter(Boolean))).sort();

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h2 className="text-sm font-bold text-surface-800">Peserta Tes ({pendaftar.length})</h2>
        <div className="flex gap-1.5 flex-wrap">
          <button onClick={() => setFilter('')}
            className={`text-xs px-2.5 py-1 rounded-md font-medium ${!filter ? 'bg-brand-600 text-white' : 'bg-surface-100 text-surface-500'}`}>Semua</button>
          {rooms.map(r => (
            <button key={r} onClick={() => setFilter(r)}
              className={`text-xs px-2.5 py-1 rounded-md font-medium ${filter === r ? 'bg-brand-600 text-white' : 'bg-surface-100 text-surface-500'}`}>{r}</button>
          ))}
        </div>
      </div>

      <p className="text-xs text-surface-400">Data otomatis dari pendaftaran PMB. Login peserta: username = NISN, password = tanggal lahir (DDMMYYYY).</p>

      {loading ? <div className="py-8 text-center"><Spinner /></div> : filtered.length === 0 ? (
        <EmptyState title="Belum ada peserta" desc="Data muncul setelah pendaftaran PMB dibuka" />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="bg-surface-50 text-surface-500 uppercase tracking-wider">
                ...
                <th className="text-left px-3 py-2">Ruang</th>
...
// ══════════════════════════════════════════════════════════════
// RUANGAN & PROKTOR — tabel + dropdown assign
// ══════════════════════════════════════════════════════════════
function RoomsPage() {
  ...
  const unassignProctor = async (proctorId: string) => {
    await PUT(`/api/admin/proctors/${proctorId}/assign`, { room_id: null });
    toast('success', 'Proktor dihapus dari ruangan');
    fetchData();
  };

  // Proktor yang belum di-assign ke ruangan manapun
  const unassignedProctors = proctors.filter(p => !p.room_id);

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h2 className="text-sm font-bold text-surface-800">Ruangan & Proktor</h2>
        <Button size="sm" loading={syncing} onClick={syncRooms}>Sinkronkan dari Data PMB</Button>
      </div>

      <p className="text-xs text-surface-400">Ruangan otomatis diambil dari data <code>ruang_tes</code> pendaftar. Assign proktor ke setiap ruangan di bawah ini.</p>
        ...
        <div className="bg-white rounded-xl border border-surface-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="bg-surface-50 text-surface-500 uppercase tracking-wider">
                <th className="text-left px-3 py-2">#</th>
                <th className="text-left px-3 py-2">Nama Ruangan</th>
                <th className="text-center px-3 py-2">Peserta</th>
                <th className="text-left px-3 py-2">Proktor</th>
                ...
                {rooms.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-surface-50/50">
                      <td className="px-3 py-2 text-surface-400">{i + 1}</td>
                      <td className="px-3 py-2 font-semibold text-surface-800">{r.room_name}</td>
                      <td className="text-center px-3 py-2"><Badge color="blue">{r.jumlah_peserta || 0}</Badge></td>
                      <td className="px-3 py-2">
                        {roomProctors.length === 0 ? (
                          <span className="text-surface-300 italic">Belum ada</span>
                        ) : (
                          <div className="space-y-1">
                            {roomProctors.map(p => (
                              <div key={p.id} className="flex items-center gap-1.5">
                                <span className="text-surface-700">{p.full_name}</span>
                                <button onClick={() => unassignProctor(p.id)} className="text-red-400 hover:text-red-600 text-[10px]">✕</button>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <td className="text-center px-3 py-2">
                        <button onClick={() => { setAssignModal(r); setSelectedProctor(''); }}
                          className="text-xs text-brand-600 font-semibold hover:underline">+ Proktor</button>
                      </td>
                    </tr>
                  );
                })}
      ...
      <Modal open={!!assignModal} onClose={() => setAssignModal(null)} title={`Assign Proktor — ${assignModal?.room_name}`} size="sm">
        {unassignedProctors.length === 0 ? (
          <p className="text-sm text-surface-500">Semua proktor sudah di-assign. Buat akun proktor baru di menu <strong>Pelaksana Tes</strong>.</p>
        ) : (
          <div className="space-y-3">
            <Select label="Pilih Proktor" value={selectedProctor} onChange={e => setSelectedProctor(e.target.value)}
              options={[{ value: '', label: '-- Pilih Proktor --' }, ...unassignedProctors.map(p => ({ value: p.id, label: p.full_name + ' (' + p.username + ')' }))]} />
...
function PelaksanaPage() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    const payload = { ...editUser, role: 'proctor' };
    const r = editUser.id ? await PUT(`/api/admin/users/${editUser.id}`, payload) : await POST('/api/admin/users', payload);
    setSaving(false);
    if (r.success) { toast('success', r.message || 'Berhasil'); setEditUser(null); fetchData(); }
    else toast('error', r.error || 'Gagal');
  };

  const deleteUser = async (id: string) => {
    await DEL(`/api/admin/users/${id}`);
    toast('success', 'Proktor dihapus');
    fetchData();
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h2 className="text-sm font-bold text-surface-800">Pelaksana Tes / Proktor ({proctors.length})</h2>
        <Button size="sm" onClick={() => setEditUser({ role: 'proctor', is_active: 1 })}>+ Tambah Proktor</Button>
      </div>

      <p className="text-xs text-surface-400">Buat akun proktor di sini, lalu assign ke ruangan di menu Ruangan & Proktor.</p>

      {loading ? <div className="py-8 text-center"><Spinner /></div> : proctors.length === 0 ? (
        <EmptyState title="Belum ada proktor" desc="Tambahkan proktor untuk mengawasi ujian" />
      ) : (
        <div className="bg-white rounded-xl border border-surface-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-surface-50">
                {proctors.map((p, i) => (
                  <tr key={p.id} className="hover:bg-surface-50/50">
                    <td className="px-3 py-2 text-surface-400">{i + 1}</td>
                    <td className="px-3 py-2 font-semibold text-surface-800">{p.full_name}</td>
                    <td className="px-3 py-2 font-mono text-surface-500">{p.username}</td>
                    <td className="px-3 py-2">{p.room_name ? <Badge color="blue">{p.room_name}</Badge> : <span className="text-surface-300">Belum di-assign</span>}</td>
                    <td className="text-center px-3 py-2">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => setEditUser({ id: p.id, username: p.username, full_name: p.full_name, role: 'proctor' })}
                          className="text-brand-600 hover:underline font-semibold">Edit</button>
                        <button onClick={() => deleteUser(p.id)} className="text-red-400 hover:text-red-600">Hapus</button>
                      </div>
                    </td>
                  </tr>
                ))}
...
// ══════════════════════════════════════════════════════════════
// EXAMS PAGE (unchanged logic, cleaned up)
// ══════════════════════════════════════════════════════════════
function ExamsPage() {
  const { toast } = useToast();
          ...
          {exams.map(e => (
            <div key={e.id} className="bg-white rounded-xl border border-surface-100 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2"><h3 className="font-bold text-sm text-surface-900 truncate">{e.title}</h3><Badge color={statusMap[e.active_status]?.color}>{statusMap[e.active_status]?.label}</Badge></div>
              ...
              <div className="flex gap-1.5 mt-2 flex-wrap">
                {(['questions','tokens','monitor','results'] as const).map(v => (
                  <button key={v} onClick={() => { setViewExamId(e.id); setViewMode(v); }}
                    className="text-xs px-2 py-1 bg-surface-50 hover:bg-surface-100 rounded-md text-surface-600 font-medium">
                    {{ questions: 'Soal', tokens: 'Token', monitor: 'Monitor', results: 'Hasil' }[v]}</button>
                ))}
              </div>
            </div>
          ))}
        ...
        {editExam && (
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <RichEditor value={editExam.rules_text || ''} onChange={v => setEditExam({ ...editExam, rules_text: v })} minHeight={100} />
            </div>
            ...
            <div className="flex flex-wrap gap-4 text-xs">
              {[{ key: 'randomize_questions', label: 'Acak Soal' },{ key: 'randomize_options', label: 'Acak Opsi' },{ key: 'is_score_visible', label: 'Tampilkan Skor' }].map(c => (
                <label key={c.key} className="flex items-center gap-1.5">
                  <input type="checkbox" checked={!!(editExam as any)[c.key]} onChange={e => setEditExam({ ...editExam, [c.key]: e.target.checked ? 1 : 0 })} className="rounded" />{c.label}
                </label>
              ))}
            </div>
            <div className="flex gap-2 justify-end pt-2">
...
// ══════════════════════════════════════════════════════════════
// QUESTIONS VIEW — upload file langsung, bukan URL manual
// ══════════════════════════════════════════════════════════════
function QuestionsView({ examId, onBack }: { examId: string; onBack: () => void }) {
  ...
  const saveQ = async () => {
    ...
    setSaving(false);
    if (r.success) { toast('success', 'Berhasil'); setEditQ(null); fetch(); } else toast('error', r.error || 'Gagal');
  };
  const delQ = async () => { if (!delTarget) return; await DEL(`/api/admin/questions/${delTarget}`); setDelTarget(null); fetch(); };
  const newQuestion = () => setEditQ({ question_text: '', question_type: 'multiple_choice', image_url: null, audio_url: null,
  ...
  const updateOption = (idx: number, field: string, value: any) => {
    if (!editQ?.options) return; const opts = [...editQ.options];
    if (field === 'is_correct') opts.forEach((o, i) => { o.is_correct = i === idx ? 1 : 0; }); else (opts[idx] as any)[field] = value;
    setEditQ({ ...editQ, options: opts });
  };
  const uploadFile = async (type: 'image' | 'audio', e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return;
    setUploading(type); const fd = new FormData(); fd.append('file', file);
    ...
    else toast('error', r.error || 'Upload gagal'); e.target.value = '';
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button size="sm" onClick={newQuestion}>+ Tambah</Button>
      </div>
      {loading ? <div className="py-8 text-center"><Spinner /></div> : questions.length === 0 ? <EmptyState title="Belum ada soal" /> : (
        <div className="space-y-2">
          {questions.map((q, i) => (
            <div key={q.id} className="bg-white rounded-xl border border-surface-100 p-3 flex items-start justify-between gap-2">
              <div className="flex-1 min-w-0">
                <p className="text-xs text-surface-400">#{i + 1}</p>
                <div className="text-sm text-surface-800 line-clamp-2" dangerouslySetInnerHTML={{ __html: q.question_text }} />
                <div className="mt-1 flex flex-wrap gap-1">
                  {q.options?.map(o => <span key={o.option_label} className={`text-xs px-1.5 py-0.5 rounded ${o.is_correct ? 'bg-brand-100 text-brand-700 font-bold' : 'bg-surface-50 text-surface-500'}`}>{o.option_label}</span>)}
                  {q.image_url && <Badge color="blue">🖼️</Badge>}{q.audio_url && <Badge color="purple">🔊</Badge>}
                </div>
              </div>
              <div className="flex gap-1">
                <Button size="sm" variant="ghost" onClick={() => setEditQ(q)}>Edit</Button>
                <button onClick={() => setDelTarget(q.id)} className="p-1.5 text-red-400 hover:text-red-600"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={!!editQ} onClose={() => setEditQ(null)} title={editQ?.id ? 'Edit Soal' : 'Tambah Soal'} size="lg">
        {editQ && (<div className="space-y-3">
          <div className="space-y-1"><label className="text-xs font-semibold text-surface-500 uppercase">Teks Soal</label>
            <RichEditor value={editQ.question_text || ''} onChange={v => setEditQ({ ...editQ, question_text: v })} minHeight={120} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1"><label className="text-xs font-semibold text-surface-500 uppercase">Gambar</label>
              {editQ.image_url ? (<div className="relative"><img src={`${API_URL}${editQ.image_url}`} alt="" className="w-full rounded-lg border max-h-32 object-cover" /><button onClick={() => setEditQ({ ...editQ, image_url: null })} className="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center">×</button></div>
              ) : (<label className="flex items-center justify-center gap-1.5 px-3 py-2 border-2 border-dashed border-surface-200 rounded-lg cursor-pointer hover:border-brand-400 text-xs text-surface-500">{uploading === 'image' ? <Spinner size={14} /> : '📷 Upload Gambar'}<input type="file" accept="image/*" className="hidden" onChange={e => uploadFile('image', e)} /></label>)}</div>
            <div className="space-y-1"><label className="text-xs font-semibold text-surface-500 uppercase">Audio</label>
              {editQ.audio_url ? (<div className="relative"><audio controls className="w-full"><source src={`${API_URL}${editQ.audio_url}`} /></audio><button onClick={() => setEditQ({ ...editQ, audio_url: null })} className="absolute top-0 right-0 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center">×</button></div>
              ) : (<label className="flex items-center justify-center gap-1.5 px-3 py-2 border-2 border-dashed border-surface-200 rounded-lg cursor-pointer hover:border-brand-400 text-xs text-surface-500">{uploading === 'audio' ? <Spinner size={14} /> : '🔊 Upload Audio'}<input type="file" accept="audio/*" className="hidden" onChange={e => uploadFile('audio', e)} /></label>)}</div>
          </div>
          {editQ.question_type === 'multiple_choice' && editQ.options && (<div className="space-y-2"><label className="text-xs font-semibold text-surface-500 uppercase">Opsi Jawaban</label>
            {editQ.options.map((o, i) => (<div key={i} className="flex items-center gap-2"><input type="radio" name="correct" checked={!!o.is_correct} onChange={() => updateOption(i, 'is_correct', true)} className="text-brand-600" /><span className="text-xs font-bold text-surface-500 w-5">{o.option_label}</span><input value={o.option_text} onChange={e => updateOption(i, 'option_text', e.target.value)} className="flex-1 px-2 py-1.5 text-sm bg-surface-50 border border-surface-200 rounded-lg outline-none focus:border-brand-500" placeholder={`Opsi ${o.option_label}`} /></div>))}
            {editQ.options.length < 5 && <button onClick={() => setEditQ({ ...editQ, options: [...editQ.options!, { option_label: 'ABCDE'[editQ.options!.length], option_text: '', image_url: null, is_correct: 0 }] })} className="text-xs text-brand-600 font-semibold hover:underline">+ Tambah Opsi</button>}
          </div>)}
...
// ══════════════════════════════════════════════════════════════
// TOKENS, RESULTS, MONITOR — ringkas
// ══════════════════════════════════════════════════════════════
function TokensView({ examId, onBack }: { examId: string; onBack: () => void }) {
  const { toast } = useToast();
  const [tokens, setTokens] = useState<any[]>([]); const [loading, setLoading] = useState(true); const [generating, setGenerating] = useState(false);
  ...
  return (<div className="space-y-3">
    <div className="flex items-center gap-3"><button onClick={onBack} className="text-xs text-surface-400 hover:text-surface-600 font-semibold">← Kembali</button><h2 className="text-xs font-bold text-surface-400 uppercase flex-1">Token</h2><Button size="sm" loading={generating} onClick={generate}>Generate</Button></div>
    {loading ? <div className="py-8 text-center"><Spinner /></div> : tokens.length === 0 ? <EmptyState title="Belum ada token" /> : (
      <div className="grid gap-2 sm:grid-cols-2">{tokens.map((t: any) => (<div key={t.id} className="bg-white rounded-xl border border-surface-100 p-4 flex items-center justify-between"><div><p className="text-sm font-semibold">{t.room_name}</p></div><span className="text-2xl font-mono font-extrabold text-brand-700 tracking-wider">{t.token_code}</span></div>))}</div>)}
  </div>);
}
...
function MonitorView({ examId, onBack }: { examId: string; onBack: () => void }) {
  ...
  return (<div className="space-y-3">
    <div className="flex items-center gap-3"><button onClick={onBack} className="text-xs text-surface-400 hover:text-surface-600 font-semibold">← Kembali</button><h2 className="text-xs font-bold text-surface-400 uppercase flex-1">Monitor ({sessions.length})</h2></div>
    {loading ? <div className="py-8 text-center"><Spinner /></div> : sessions.length === 0 ? <EmptyState title="Belum ada sesi" /> : (
      <div className="bg-white rounded-xl border border-surface-100 overflow-hidden overflow-x-auto"><table className="w-full text-xs"><thead><tr className="bg-surface-50 text-surface-500 uppercase"><th className="text-left px-3 py-2">Peserta</th><th className="text-left px-3 py-2">Ruangan</th><th className="text-center px-2 py-2">Status</th><th className="text-center px-2 py-2">Warn</th></tr></thead>
        <tbody className="divide-y divide-surface-50">{sessions.map((s: any) => { const isOn = s.status==='active'&&(Date.now()-new Date(s.last_heartbeat).getTime())<30000; const done = s.status==='submitted'||s.status==='force_submitted';
          return (<tr key={s.id}><td className="px-3 py-2 font-semibold">{s.full_name}</td><td className="px-3 py-2 text-surface-500">{s.room_name}</td><td className="text-center px-2 py-2"><Badge color={done?'gray':isOn?'green':'red'}>{done?'Selesai':isOn?'Online':'Offline'}</Badge></td><td className="text-center px-2 py-2">{s.cheat_warnings>0?<span className="text-red-600 font-bold">{s.cheat_warnings}</span>:'0'}</td></tr>);})}</tbody></table></div>)}
  </div>);
}
